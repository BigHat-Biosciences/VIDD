...
def _run_inference(args, result_save_folder: str, device: torch.device) -> dict:
    ...
    # Final binders generated by the inference phase. best_of_n_test produces
    # exactly args.batch_size sequences (best-of-N collapses N candidates to
    # one per batch slot), and ab_af2_reward saves one PDB per slot. Cross-check
    # against the on-disk PDB count so we report what's actually saved.
    saved_dir = os.path.join(result_save_folder, "saved_proteins")
    saved_pdbs = (
        sorted(p for p in os.listdir(saved_dir) if p.endswith(".pdb"))
        if os.path.isdir(saved_dir) else []
    )
    n_final_binders = len(saved_pdbs) or args.batch_size
